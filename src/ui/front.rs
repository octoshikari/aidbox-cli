...
pub fn draw<B: Backend>(f: &mut Frame<B>, app: &mut App) {
  ...
  match app.tabs.index {
    0 => draw_rest(f, app, chunks[1]),
    1 => draw_db(f, app, chunks[1]),
    _ => {},
  };
}

fn draw_rest<B>(f: &mut Frame<B>, app: &mut App, area: Rect)
where
  B: Backend,
{
  let chunks = Layout::default()
...
fn draw_db<B>(f: &mut Frame<B>, app: &mut App, area: Rect)
where
  B: Backend,
{
  let chunks = Layout::default()
  ...
  let input = Paragraph::new(lines).block(
    Block::default()
      .borders(Borders::ALL)
      .title(Span::styled("Query", up_style)),
  );

  f.render_widget(input, chunks[0]);

  let rows1 = app.servers.iter().map(|s| {
